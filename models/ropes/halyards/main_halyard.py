...
class MainHalyard(Halyard):
    default_upper_termination = Termination(term_type="Covered Splice", hardware="Thimbled Long-D Shackle")
    default_lower_termination = Termination(term_type="Pull Through Whipping", hardware=None)
    default_colour = "Solid White"
    ...
    def calc_length(self) -> float:
        ...
        raw_length = (
            (self.yacht.saildata.main_p * 2)
            + sqrt(self.yacht.saildata.main_p ** 2 + self.yacht.saildata.main_e ** 2)
            + self.led_aft
            + self.safety_margin
        )
