...
class Genoa(BaseSail):
    ...
    def __init__(self, saildata, luff=None, leech=None, foot=None, overlap_percent=None, yacht_id=None):
        ...
        default_luff = sqrt(saildata["genoa_i"] ** 2 + saildata["genoa_j"] ** 2)
        luff = luff if luff is not None else default_luff
        overlap = overlap_percent if overlap_percent is not None else 100
        foot = foot if foot is not None else saildata["genoa_j"] * (overlap / 100)
    ...
    def area(self) -> float:
        """
        Calculate the area of the genoa foresail in square meters.
        Returns:
            float: The area of the sail (m^2).
        """
        # Area is always 0.5 * luff * foot for a genoa, overlap is already included in foot
