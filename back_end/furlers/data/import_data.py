...
# Ensure src and data are in sys.path for imports
...
# Use explicit importlib for robust local imports
...
# Import database and temp_data robustly
DATABASE_PATH = os.path.join(SRC_PATH, "database.py")
TEMP_DATA_PATH = os.path.join(DATA_PATH, "temp_data.py")
database = import_module_from_path("database", DATABASE_PATH)
temp_data = import_module_from_path("temp_data", TEMP_DATA_PATH)

create_tables = database.create_tables
get_connection = database.get_connection

FURLEX_PART_NUMBERS = temp_data.FURLEX_PART_NUMBERS
FURLEX_LINK_PLATES = temp_data.FURLEX_LINK_PLATES
PROFURL_SELECTION_CRITERIA = temp_data.PROFURL_SELECTION_CRITERIA
PROFURL_REQUIRES_SWAGELES_EYE = temp_data.PROFURL_REQUIRES_SWAGELES_EYE
PROFURL_PART_NUMBERS = temp_data.PROFURL_PART_NUMBERS
PROFURL_TURNBUCKLE_CYLINDERS = temp_data.PROFURL_TURNBUCKLE_CYLINDERS
PROFURL_LINK_PLATES = temp_data.PROFURL_LINK_PLATES
PROFURL_PREFEEDER_PART_NUMBERS = temp_data.PROFURL_PREFEEDER_PART_NUMBERS
PROFURL_REEFING_KIT_PART_NUMBERS = temp_data.PROFURL_REEFING_KIT_PART_NUMBERS
FACNOR_SELECTION_CRITERIA = temp_data.FACNOR_SELECTION_CRITERIA
MKIV_PART_NUMBERS = temp_data.MKIV_PART_NUMBERS
MKIV_OCEAN_PART_NUMBERS = temp_data.MKIV_OCEAN_PART_NUMBERS
MKIV_UNDERDECK_PART_NUMBERS = temp_data.MKIV_UNDERDECK_PART_NUMBERS
...
def import_harken_selection_criteria():
    try:
        from temp_data import HARKEN_SELECTION_CRITERIA
    except ImportError:
        return
...
def import_facnor_requires_eye_turnbuckle():
    from temp_data import FACNOR_REQUIRES_EYE_TURNBUCKLE
    with get_connection() as conn:
        count = 0
