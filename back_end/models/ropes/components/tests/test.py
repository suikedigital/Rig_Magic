from back_end.models.ropes.components.rope_core import RopeCore
from back_end.models.ropes.components.rope_cover import RopeCover
from back_end.models.ropes.components.rope_construction import RopeConstruction
...
assert (
    abs(cover_braid.contribution(2000) - 1000) < 1e-6
), "Braid cover contribution failed"
assert (
    abs(cover_dyneema.contribution(2000) - 1500) < 1e-6
), "Dyneema cover contribution failed"
...
assert (
    abs(rope.total_break_strength() - (2800 + 1400)) < 1e-6
), "Total break strength with cover failed"
...
assert (
    abs(rope_no_cover.total_break_strength() - 2800) < 1e-6
), "Total break strength without cover failed"
