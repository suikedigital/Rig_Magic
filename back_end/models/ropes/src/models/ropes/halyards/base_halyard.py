...
class Halyard(Rope, ABC):
    ...
    def calc_load(self, HALYARD_TO_SAIL, wind_speed_knots):
        ...
        print(
            f"[DEBUG] {self.__class__.__name__}: sail_names from HALYARD_TO_SAIL = {sail_names}"
        )
        max_load = 0
        for sail_name in sail_names:
            sail = self.sail_service.get_sail(self.yacht_id, sail_name)
            print(
                f"[DEBUG] {self.__class__.__name__}: get_sail({self.yacht_id}, {sail_name}) returned: {sail}"
            )
            if sail is None:
                continue
            sail_type = self._normalize_sail_type(sail["name"])
            print(
                f"[DEBUG] {self.__class__.__name__}: normalized sail_type = {sail_type}"
            )
            aero_force_newtons = self.sail_service.get_aero_force(
                self.yacht_id, sail_type, wind_speed_knots
            )
            print(
                f"[DEBUG] {self.__class__.__name__}: get_aero_force({self.yacht_id}, {sail_type}, {wind_speed_knots}) = {aero_force_newtons}"
            )
            if aero_force_newtons is None:
                raise ValueError(
                    f"Aero force could not be calculated for sail {sail['name']} on yacht {self.yacht_id}"
                )
            load_n = (
                (aero_force_newtons * sin(radians(self.halyard_angle_deg)))
                * self.halyard_load_safety_factor
            ) * self.dynamic_load_safety_factor
            load_kg = load_n / 9.80665
            print(
                f"[DEBUG] {self.__class__.__name__}: load_kg for sail {sail['name']} = {load_kg}"
            )
            if load_kg > max_load:
                max_load = load_kg
        if max_load == 0:
            print(
                f"[DEBUG] {self.__class__.__name__}: No valid sails found for {self.__class__.__name__} on yacht {self.yacht_id}"
            )
            raise ValueError(
                f"No valid sails found for {self.__class__.__name__} on yacht {self.yacht_id}"
            )
    ...
    def calc_diameter(self, HALYARD_TO_SAIL, wind_speed_knots) -> int:
        ...
        print(
            f"[DEBUG] {self.__class__.__name__}: required working load = {required_wl:.2f} kg"
        )
        ...
        for d in diameters:
            if self.construction_type:
                ...
                print(
                    f"[DEBUG] {self.__class__.__name__}: diameter {d} mm, break strength = {break_strength:.2f} kg"
                )
                if break_strength >= required_wl:
                    ...
                    return d
        raise ValueError(
            f"No suitable diameter found for {self.type} with required working load {required_wl:.1f} kg."
        )
