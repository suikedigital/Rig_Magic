...
class UserYachtDatabase:
    ...
    def update(self, yacht_id, yacht_data):
        ...
        self.conn.execute(
            f"UPDATE user_yachts SET {set_clause} WHERE id = ?", values
        )
        self.conn.commit()

    def get_by_id(self, yacht_id):
        cursor = self.conn.execute("SELECT * FROM user_yachts WHERE id = ?", (yacht_id,))
        return cursor.fetchone(), [desc[0] for desc in cursor.description]

    def get_by_id_and_owner(self, yacht_id, owner_id):
        cursor = self.conn.execute(
            "SELECT * FROM user_yachts WHERE id = ? AND owner_id = ?", (yacht_id, owner_id)
        )
        return cursor.fetchone(), [desc[0] for desc in cursor.description]

    def list(self, owner_id=None):
        if owner_id:
            cursor = self.conn.execute("SELECT * FROM user_yachts WHERE owner_id = ?", (owner_id,))
        else:
